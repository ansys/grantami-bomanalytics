...
def test_add_bom(query_type, connection):
    query = query_type(connection).set_bom("TEST BOM")
...
def test_add_bom_wrong_type(query_type, connection):
    with pytest.raises(TypeError) as e:
        query_type(connection).set_bom(12345)
    assert "Incorrect type for value" in str(e.value)
    with pytest.raises(TypeError) as e:
        query_type(connection).set_bom(bom=12345)
    assert "Incorrect type for value" in str(e.value)
