...
class TestRecordArgManager:
    def test_uninitialized_configuration(self):
        ...
        assert am.record_type_name is None
    ...
    def test_no_record_type_raises_runtime_error(self):
        ...
        assert '"record_type_name" must be populated before record arguments can be generated.' in str(e.value)

    def test_no_batch_size_raises_runtime_error(self):
        am = queries._RecordArgumentManager(record_type_name="TEST_RECORD")
        with pytest.raises(RuntimeError) as e:
            next(am.batched_record_arguments)
    ...
    def test_initialized_arg_manager_no_records(self):
        am = queries._RecordArgumentManager(batch_size=100, record_type_name="TEST_RECORD")
    ...
    def test_argument_batching(self, number_of_records, batch_size):
        am = queries._RecordArgumentManager(batch_size=batch_size, record_type_name="TEST_RECORD")
    ...
    def test_repr(self):
        am = queries._RecordArgumentManager(batch_size=100, record_type_name="TEST_NAME")
...
class TestBomArgManager:
    ...
    def test_add_bom(self, bom):
        am = queries._BomArgumentManager(bom=bom)
        assert am.bom == bom
