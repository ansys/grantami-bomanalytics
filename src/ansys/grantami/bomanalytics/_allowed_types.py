    ...
    *allowed_types
    ...
    def decorator(method: Type[T]) -> Callable[[Any, Any], T]:
        ...
        def check_types(*args: Any, **kwargs: Any) -> T:
            ...
            if len(values) != 1:
                raise TypeError(f"The allowed_types decorator can only be used on a method called with "
...
def _check_type(value_obj: Any, type_obj: Any) -> None:
    ...
    if isinstance(type_obj, (list, tuple)):
        ...
        if len(type_obj) == 1:
            ...
            raise ValueError("List or tuple containers must be either of length 1 or the same length as the "
        ...
        if len(type_obj) != 1:
            raise ValueError("Sets must be homogeneous: they can only contain a single object against "
        ...
        if len(type_obj) != 1:
            raise ValueError("Dictionaries must be homogeneous: they can only contain a single object "
