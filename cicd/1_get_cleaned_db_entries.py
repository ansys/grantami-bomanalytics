...
from cicd._config import MI_URL, RS_DB_KEY, DATA_FILENAME, TABLE_INFORMATION, EXTRA_ATTRIBUTES, RENAMED_ATTRIBUTES
...
info = {}
logger.info(f"Reading records and attributes from database '{RS_DB_KEY}'")
logger.info("Getting Table Information")
tables = tables_api.get_tables(database_key=RS_DB_KEY)
...
for table_name, table_details in TABLE_INFORMATION.items():
    ...
    table_layouts = layouts_api.get_layouts(database_key=RS_DB_KEY, table_guid=table_info.guid)
    ...
    sections_response = layout_sections_api.get_layout_sections(
        database_key=RS_DB_KEY, table_guid=table_info.guid, layout_guid=layout_item.guid, show_full_detail=True
    )
    ...
    if table_name in EXTRA_ATTRIBUTES:
        ...
        table_attributes = attributes_api.get_attributes(database_key=RS_DB_KEY, table_guid=table_info.guid)
