...
def test_add_material_ids(query_type, material_ids, connection):
    query = query_type(connection).add_material_ids(material_ids)
...
def test_add_material_ids_wrong_type(query_type, connection):
    with pytest.raises(TypeError) as e:
        query_type(connection).add_material_ids("Strings are not allowed")
    assert 'Incorrect type for value "Strings are not allowed"' in str(e.value)
    with pytest.raises(TypeError) as e:
        query_type(connection).add_material_ids(material_ids="Strings are not allowed")
    assert 'Incorrect type for value "Strings are not allowed"' in str(e.value)
