...
class BomAnalyticsClient(ApiClient):
    ...
    def run(self, query):  # type: ignore[no-untyped-def]
        ...
        :class:`~ansys.grantami.bomanalytics.GrantaMIException`
            If the server encounters an error while processing the query with a severity of "critical". This indicates that Granta MI
            is running and the BoM Analytics Service is available, but the query could not be run; probably
